...
- `image` - отдельная картинка между текстовыми блоками; поля: `image`, `alt`, `caption`, `showBorder`, `size`. В статье картинки выводятся целиком и сохраняют свои исходные пропорции; рамку можно отключить отдельно у каждой картинки.
