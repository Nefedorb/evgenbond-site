...
- Добавлен блочный конструктор статей в Pages CMS: `text`, `image`, `video`, `code`, `quote`, `divider`, `download`.
...
- У `image`-блоков и обложки настраиваются рамка, её толщина, стиль, HEX-цвет и скругление.
- В rich-text блок можно вставлять изображения из буфера обмена через `Ctrl+V`.
- Sharp автоматически создаёт WebP-варианты загруженных JPEG/PNG при сборке; оригиналы остаются fallback.
...
- `scripts/blog/images.mjs` - генерация WebP-вариантов через Sharp.
...
- `.pages.yml` содержит актуальные поля CMS для файлов, вставки изображений и оформления рамок.
